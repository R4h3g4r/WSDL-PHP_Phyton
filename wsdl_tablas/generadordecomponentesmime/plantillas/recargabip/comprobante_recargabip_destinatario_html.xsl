	...
	<xsl:template match="comprobante">
		...
		<table width="500" border="0" cellpadding="0" cellspacing="0" style="font:normal 12px Arial, Helvetica, sans-serif;border:solid 1px #CCC;padding:10px"> 
		  ...
		  <tr> 
			...
			<p><strong>Estimado (a) <xsl:value-of select="nombre-beneficiario" /></strong>:</p> 
			<xsl:choose>
				<xsl:when test="$canal = '100'">
					<p>Con fecha <xsl:value-of select="fecha-transaccion" /> se ha realizado una recarga a tu tarjeta bip! El detalle de esta operacion es el siguiente:</p> 
				</xsl:when>
				<xsl:otherwise>
					<p>Con fecha <xsl:value-of select="fecha-transaccion" /> se ha realizado una recarga a su tarjeta bip! El detalle de esta operacion es el siguiente:</p> 
				</xsl:otherwise>
			</xsl:choose>
			...
			<xsl:choose>
				<xsl:when test="$canal = '100'">
				      <p>Recuerda que debes activar el monto transferido a tu Tarjeta en los equipos de carga bip! ubicado en sucursales del banco o en los lugares senalizados y habilitados para dicho efecto.<br /> 
				      Si no efectuas dicha activacion, el monto transferido no se activara en tu tarjeta.</p> 
				</xsl:when>
				<xsl:otherwise>
				      <p>Recuerde que debe activar el monto transferido a su Tarjeta en los equipos de carga bip! ubicado en sucursales del banco o en los lugares senalizados y habilitados para dicho efecto.<br /> 
				      Si no efectua dicha activacion, el monto transferido no se activara en su tarjeta.</p> 
				</xsl:otherwise>
			</xsl:choose>
			<xsl:choose>
				<xsl:when test="$canal = '100' or $canal = '905'">
