...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="comprobante">
		...
		<xsl:text>&#xA;</xsl:text>
		<xsl:text>Nombre Automotora           : </xsl:text><xsl:value-of select="nombre-destinatario"/>
		<xsl:text>&#xA;</xsl:text>
		<xsl:text>E-mail Automotora           : </xsl:text><xsl:value-of select="correo-destinatario"/>
		<xsl:text>&#xA;</xsl:text>
