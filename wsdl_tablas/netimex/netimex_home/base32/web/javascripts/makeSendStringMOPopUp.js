function makeSendStringMOPopUp() {
var str = "";
	...
	for(i=0;i < window.document.<%getPageName()%>_form.length;i++)
	{
		if(window.document.<%getPageName()%>_form.elements[i] != null )
		{
			...
			//if(fName == "cmbDepname" || fName == "txtDepdefemail" || fName == "txtDepemaillist")
			{
				...
				if(fName == "cmbDepname" || fName == "depNameHidden"){
					fValue = javascript:GetElemValueMO(window.document.<%getPageName()%>_form.elements[i]);
					if(fValue == null || fValue == ""){continue;}
					fName = "depname";
				}
				if(fName == "txtDepdefemail"){fName = "depdefemail";}
				if(fName == "txtDepemaillist"){fName = "depemaillist";}
				fValue = javascript:GetElemValueMO(window.document.<%getPageName()%>_form.elements[i]);
...
function GetElemValueMO(Elem) { // This function retrun the form elment value according to his type
	var i;
	if (Elem == null) return "";
	if (Elem.type != null){
		var sType = Elem.type;      
