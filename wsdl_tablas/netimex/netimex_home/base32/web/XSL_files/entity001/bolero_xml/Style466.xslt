<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cmp=" http://www.bolero.net/io/xmlns/header/1.0">
	...
	<xsl:template match="DocumentaryCreditNotification|Body|BoleroOrSWIFTFormat">
		<xsl:apply-templates/>
	</xsl:template>
				...
				<td>
					<table border="0" cellspacing="0" width="{$TableWidth}">
						<tr align="left"><td colspan="2">
							<H1><xsl:text>Request for Payment of Bank Charges</xsl:text></H1>
						</td></tr>
						<tr><td>
							<a name="CHGS">
							<xsl:apply-templates select="RequestForPaymentOfBankCharges"/></a>
						</td></tr>
					</table>
				</td>
			</tr>
