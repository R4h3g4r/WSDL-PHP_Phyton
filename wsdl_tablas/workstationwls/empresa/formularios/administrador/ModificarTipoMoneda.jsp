...
<c:set value="" var="nombreDeLaMoneda" scope="page" />
<c:if test="${!empty param.nombreDeLaMoneda}">
	<c:set value="${param.nombreDeLaMoneda}" var="nombreDeLaMoneda" scope="page" />
</c:if>
<c:set value="${param.idMoneda}" var="idMoneda" scope="page" />
<c:if test="${!empty requestScope.tipoDeMoneda}">
	<c:set value="${requestScope.nombreDeLaMoneda}" var="nombreDeLaMoneda" scope="page" />
	<c:set value="${requestScope.tipoDeMoneda}"     var="idMoneda"         scope="page" />	
</c:if>
...
<head>
	...
	function fnModificar(){
		...
		if(trim(document.forms[0].nombreDeLaMoneda.value) == ''){
			document.getElementById('error').style.display = '';
			document.getElementById('error').innerHTML = 'Debe ingresar un Nombre a la Moneda';
			return;
		}
...
<body>
	...
	<form action="administracionFormulario.do">
		<input type="hidden" name="metodo" value="modificarTipoMoneda">
		<input type="hidden" name="tipoDeMoneda" value="<c:out value="${idMoneda}"/>">
				...
				<td id="pestana-fd">
					<table border="0" cellpadding="0" cellspacing="0" id="pestana">
						<tr>
							...
							<th style="cursor: pointer" onclick="document.location.href='/workstationwls/empresa/formularios/administrador/administracionFormulario.do?metodo=desplegarAdministracionTipoMoneda'"><fmt:message key="adm.monedas"/></th>
		...
		<fieldset>
		<legend style="font-size:13px;color: black;"><b><fmt:message key="adm.moneda.mod"/><b></legend>
		<table id="lista-bonita" width="100%" height="100%">
			<tr>
				<th width="20%"><fmt:message key="adm.moneda.nom"/></th><td><input class="caja280" type="text" name="nombreDeLaMoneda" value="<c:out value="${nombreDeLaMoneda}"/>"> </td>
			</tr>
		</table>
		<div>
...
<%--
 ******************************************************************************************************
 * Archivo           ModificarTipoMoneda.jsp
 * Descripción       Modifica tipo de moneda
 * Versión           1.0
