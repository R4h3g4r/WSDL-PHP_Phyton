...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template name="contenido">
				...
				<fo:table-body>
                    ...
                    <xsl:if test="1>count(Info/RiesgoCliente/RiesgoCliente)">
	                    <fo:table-row>
							...
							<xsl:call-template name="celda-cabecera">
								<xsl:with-param name="texto" select="'Pl(%) 5 años'"/>
							</xsl:call-template>
						</fo:table-row>
		...
		<fo:block>
			<xsl:call-template name="subTitulo">
				<xsl:with-param name="titulo" select="'Pérdida Esperada (PE)'"/>
			</xsl:call-template>
		...
		<fo:block>
			<xsl:call-template name="subTitulo">
				<xsl:with-param name="titulo" select="'Calificación Banco'"/>
			</xsl:call-template>
			<fo:table table-layout="fixed" space-before="3mm">
				...
				<fo:table-body>
				    <xsl:if test="count(Info/RiesgoCliente/RiesgoCliente)>0">
                        <fo:table-row>
                            ...
                            <xsl:call-template name="celda-cabecera">
                                <xsl:with-param name="texto" select="'Tipo Calificación'"/>
                            </xsl:call-template>
                        </fo:table-row>
                        <xsl:for-each select="Info/RiesgoCliente/RiesgoCliente">
                            <xsl:if test="position()=1">
...
<xsl:if test="1>count(Info/RiesgoCliente/RiesgoCliente)">
		...
		<fo:block>
			<xsl:call-template name="subTitulo">
				<xsl:with-param name="titulo" select="'Clasificación Privada'"/>
			</xsl:call-template>
			<fo:table table-layout="fixed" space-before="3mm">
		...
		<fo:block>
			<xsl:call-template name="subTitulo">
				<xsl:with-param name="titulo" select="'Deuda Garantía'"/>
			</xsl:call-template>
			<fo:table table-layout="fixed" space-before="3mm">
				...
				<fo:table-body>
					<fo:table-row>
						...
						<xsl:call-template name="celda-cabecera">
							<xsl:with-param name="texto" select="'Garantías (miles $)'"/>
						</xsl:call-template>
						<xsl:call-template name="celda-cabecera">
