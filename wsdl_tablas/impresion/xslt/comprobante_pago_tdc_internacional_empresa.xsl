 ...
 <xsl:template match="encabezado">
   <fo:block text-align="right" line-height="30pt">
 	     <fo:external-graphic>
		    ...
		    <xsl:value-of select="rutalogo"/>02-logo-bci.gif
	         </xsl:attribute>
	     </fo:external-graphic>
        </fo:block>
   <fo:block font-weight="bold" font-size="15pt" color=" black">
           Comprobante de pago de Tarjeta de Cr&#233;dito Internacional<xsl:if test="detalle/pendiente = &quot;si&quot;"> Pendiente</xsl:if>
   </fo:block>
   ...
   <fo:table table-layout="fixed">
    ...
    <fo:table-body>
        <fo:table-row>
  	    <fo:table-cell>
		<xsl:if test="detalle/pendiente = &quot;no&quot;">
		...
		<fo:table table-layout="fixed">
		    ...
		    <fo:table-body>
		       <fo:table-row >
		        ...
		        <fo:table-row >
                    ...
                    <fo:table-cell>
                        <fo:block font-weight="bold"  color=" black">
                             <xsl:value-of select="format-number(rutempresa, '', 'latin')"/>-<xsl:value-of select="dvempresa"/>
                        </fo:block>
                    </fo:table-cell>
                </fo:table-row>
                
                <xsl:if test="apoderado !=&quot;&quot;">
                    <fo:table-row>
                       <fo:table-cell>
                           <fo:block font-weight="bold"  color=" black" text-align="right">
                               Apoderado:
		        ...
		        <fo:table-row>
		              ...
		              <fo:table-cell>
		                  <fo:block font-weight="bold"  color=" black">
		                       XXXX-<xsl:value-of select="numerotdc"/> <xsl:text> </xsl:text> <xsl:value-of select="usuario"/> 
		                  </fo:block>
		              </fo:table-cell>
		        </fo:table-row> 
		        <fo:table-row>
		         ...
		         <xsl:if test="muestraValorDolar =&quot;si&quot;">
		         <fo:table-row>
                       <fo:table-cell>
                          <fo:block font-weight="bold"  color=" black" text-align="right">
                               Valor D&#243;lar:
                       ...
                       <fo:table-cell>
                          <fo:block font-weight="bold"  color=" black" >
                               $<xsl:value-of select="valorDolar"/>
                          </fo:block>
                       </fo:table-cell>
                 </fo:table-row>
                 </xsl:if>
		         
		 	<xsl:if test="pendiente =&quot;no&quot;">
		            <fo:table-row>
		               <fo:table-cell>
		                   <fo:block font-weight="bold"  color=" black" text-align="right">
		                       Monto Pagado:
			...
			<xsl:if test="pendiente =&quot;si&quot;">
                    <fo:table-row>
                       ...
                       <fo:table-cell>
                           <fo:block font-weight="bold"  color=" black">
                               US $<xsl:value-of select="montoDolares"/>
                           </fo:block>
                       </fo:table-cell>
                    </fo:table-row>
            </xsl:if>  
