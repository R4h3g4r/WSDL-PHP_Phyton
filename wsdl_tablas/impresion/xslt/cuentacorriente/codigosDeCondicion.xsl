		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			...
			<fo:layout-master-set>
				<fo:simple-page-master master-name="pagina"
					page-height="30cm" page-width="21cm" margin-top="1cm"
	...
	<xsl:template name="contenido">
						...
						<fo:table-cell>
							...
							<fo:block font-size="10pt"
								...
								<fo:inline color="{$txt-azul-claro}">
									<xsl:value-of select="impresion/rut" />
								</fo:inline>
							</fo:block>
							<!-- rut cliente -->
						</fo:table-cell>
		...
		<fo:block space-after="3mm">
			<fo:leader color="{$lineas}" leader-pattern="rule"
				leader-length="{$tam-separacion}" />
		</fo:block>
		<!-- Separador -->

		<!-- ejecutivo -->
		<fo:block font-size="9pt" text-align="left" color="{$txt}">
			<fo:inline font-weight="normal" space-before="30pt">
				Ejecutivo:
			</fo:inline>
			<fo:inline font-weight="bold">
				<xsl:value-of select="impresion/ejecutivo" />
				&#0160;&#0160;&#0160;&#0160;&#0160;&#0160;&#0160;&#0160;&#0160;&#0160;&#0160;&#0160;
			</fo:inline>
			<fo:inline font-weight="normal" space-before="30pt">
				Oficina:
			...
			<fo:inline font-weight="bold">
				<xsl:value-of select="impresion/oficina" />
			</fo:inline>
		</fo:block>
		<!-- ejecutivo -->
					...
					<fo:table-row>
						<fo:table-cell>
							...
							<fo:block font-size="15pt"
								...
								color="{$txt-azul-claro}">
								Otras Condiciones de la Cuenta.
							</fo:block>
							<!-- titulo -->
						</fo:table-cell>
						<fo:table-cell>
							<fo:block font-size="10pt"
								space-after="1mm" text-align="right"
								color="{$txt}">
							<fo:inline color="{$txt}">
								Cuenta N°:
							</fo:inline>
							<fo:inline color="{$txt}" font-weight="bold">
								<xsl:value-of select="impresion/numeroCuenta" />
							</fo:inline>
							</fo:block>
						</fo:table-cell>
					</fo:table-row>
		...
		<fo:block font-size="7pt" space-after="1mm"
			...
			<fo:table table-layout="fixed">
				...
				<fo:table-body>
					...
					<xsl:for-each select="impresion/condiciones/condicion">
						<fo:table-row>
							<fo:table-cell
								border="0.5pt solid {$lineas}">
								<fo:block font-weight="normal"
