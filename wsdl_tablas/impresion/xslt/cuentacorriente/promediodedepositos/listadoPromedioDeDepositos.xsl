...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template name="contenido">
		...
		<fo:block>
			<fo:table table-layout="fixed">
				...
				<fo:table-body>
					<fo:table-row>
						<fo:table-cell>
							<fo:block font-size="15pt" font-weight="bold" text-align="left" color="{$txt-azul-claro}">Saldos Promedios Depósito</fo:block>
						</fo:table-cell>
						<fo:table-cell>
								<fo:block font-size="10pt" font-weight="bold" space-after="1mm" text-align="right" color="{$txt}">
									<fo:inline color="{$txt}">Nº Cuenta Corriente:</fo:inline>
									<fo:inline color="{$txt-azul-oscuro}">
										<xsl:value-of select="Info/Cliente/CtaCte"/>
									</fo:inline>
							</fo:block>
						</fo:table-cell>
					</fo:table-row>
		...
		<fo:block>
			<fo:table table-layout="fixed" space-before="3mm"  >
				...
				<fo:table-body>
					<fo:table-row>
						<xsl:call-template name="celda-cabecera-izq">
							<xsl:with-param name="texto" select="'Nombre del Titular de la Cuenta Corriente'"/>
						</xsl:call-template>
						<xsl:call-template name="celda-cabecera">
							<xsl:with-param name="texto" select="'Fecha de Apertura'"/>
						</xsl:call-template>
					</fo:table-row>
					<fo:table-row>
						<xsl:call-template name="celda-izq-tot">
							<xsl:with-param name="texto" select="Info/SaldosPromediosDepositos/DatosTitular/NombreTitular"/>
						</xsl:call-template>
						<xsl:call-template name="celda-cen-tot">
							<xsl:with-param name="texto">
								<xsl:call-template name="formato-fecha">
									<xsl:with-param name="texto" select="Info/SaldosPromediosDepositos/DatosTitular/FechaApertura"/>
								</xsl:call-template>
							</xsl:with-param>
						</xsl:call-template>
					</fo:table-row>
			...
			<fo:table table-layout="fixed" space-before="3mm">
				...
				<fo:table-body>
					<fo:table-row>
						<xsl:call-template name="celda-cabecera-izq">
							<xsl:with-param name="texto" select="'Período'"/>
						</xsl:call-template>
						<xsl:call-template name="celda-cabecera-der">
							<xsl:with-param name="texto" select="'Depósitos'"/>
						</xsl:call-template>
						<xsl:call-template name="celda-cabecera-der">
							<xsl:with-param name="texto" select="'Saldos Promedios Contables'"/>
						</xsl:call-template>
						<xsl:call-template name="celda-cabecera-der">
							<xsl:with-param name="texto" select="'Saldos Promedios Disponibles'"/>
						</xsl:call-template>
					</fo:table-row>	
					<xsl:for-each select="Info/SaldosPromediosDepositos/DetalleSaldos">
						<fo:table-row>
							<xsl:call-template name="celda-izq">
								<xsl:with-param name="texto" select="Periodo"/>
							</xsl:call-template>
							
							<xsl:call-template name="celda">
								<xsl:with-param name="texto" select="Deposito"/>
							</xsl:call-template>
							<xsl:call-template name="celda">
								<xsl:with-param name="texto" select="SaldoContable"/>
							</xsl:call-template>
							<xsl:call-template name="celda">
								<xsl:with-param name="texto" select="SaldoDisponible"/>
							</xsl:call-template>
						</fo:table-row>
					</xsl:for-each>		
					<xsl:for-each select="Info/SaldosPromediosDepositos/Totales">
						<fo:table-row>
							<xsl:call-template name="celda-izq-tot">
								<xsl:with-param name="texto" select="'Totales Trimestre'"/>
							</xsl:call-template>
							<xsl:call-template name="celda-der-tot">
								<xsl:with-param name="texto" select="TotalDeposito"/>
							</xsl:call-template>
							<xsl:call-template name="celda-der-tot">
								<xsl:with-param name="texto" select="TotalContable"/>
							</xsl:call-template>
							<xsl:call-template name="celda-der-tot">
								<xsl:with-param name="texto" select="TotalDisponible"/>
							</xsl:call-template>
						</fo:table-row>
					</xsl:for-each>			
				</fo:table-body>
	...
	<xsl:template name="celda">
		...
		<xsl:template name="celda-cen-tot">
		<xsl:param name="texto"/>
		<fo:table-cell border-top-style="solid" border-top-width="thin" border-top-color="{$borde-tabla}" border-bottom-style="solid" border-bottom-width="thin" border-bottom-color="{$borde-tabla}" border-left-style="solid" border-left-width="thin"
		               border-left-color="{$borde-tabla}" border-right-style="solid" border-right-width="thin" border-right-color="{$borde-tabla}">
			<fo:block font-size="8pt" font-weight="bold" text-align="center" space-before="{$espacio-texto-celda-top}" >
				<fo:inline>
					<xsl:value-of select="$texto"/>
				</fo:inline>
			</fo:block>
	...
	<xsl:template name="celda-izq-tot">
		<xsl:param name="texto"/>
		<fo:table-cell border-top-style="solid" border-top-width="thin" border-top-color="{$borde-tabla}" border-bottom-style="solid" border-bottom-width="thin" border-bottom-color="{$borde-tabla}" border-left-style="solid" border-left-width="thin"
		               border-left-color="{$borde-tabla}" border-right-style="solid" border-right-width="thin" border-right-color="{$borde-tabla}">
			<fo:block font-size="8pt" font-weight="bold" text-align="left" space-before="{$espacio-texto-celda-top}" >
				<fo:inline>
					<xsl:value-of select="$texto"/>
				</fo:inline>
			</fo:block>
	...
	<xsl:template name="celda-der-tot">
		<xsl:param name="texto"/>
		<fo:table-cell border-top-style="solid" border-top-width="thin" border-top-color="{$borde-tabla}" border-bottom-style="solid" border-bottom-width="thin" border-bottom-color="{$borde-tabla}" border-left-style="solid" border-left-width="thin"
		               border-left-color="{$borde-tabla}" border-right-style="solid" border-right-width="thin" border-right-color="{$borde-tabla}">
			<fo:block font-size="8pt" font-weight="bold" text-align="right" space-before="{$espacio-texto-celda-top}" >
				<fo:inline>
					<xsl:value-of select="$texto"/>
				</fo:inline>
			</fo:block>
	...
	<xsl:template name="celda-cabecera-izq">
		<xsl:param name="texto"/>
		<fo:table-cell background-color="{$bgcolor}" border-top-style="solid" border-top-width="thin" border-top-color="{$borde-tabla}" border-bottom-style="solid" border-bottom-width="thin" border-bottom-color="{$borde-tabla}" border-left-style="solid"
		               border-left-width="thin" border-left-color="{$borde-tabla}" border-right-style="solid" border-right-width="thin" border-right-color="{$borde-tabla}">
	...
	<xsl:template name="celda-cabecera-der">
		<xsl:param name="texto"/>
		<fo:table-cell background-color="{$bgcolor}" border-top-style="solid" border-top-width="thin" border-top-color="{$borde-tabla}" border-bottom-style="solid" border-bottom-width="thin" border-bottom-color="{$borde-tabla}" border-left-style="solid"
		               border-left-width="thin" border-left-color="{$borde-tabla}" border-right-style="solid" border-right-width="thin" border-right-color="{$borde-tabla}">
			<fo:block text-align="right" font-weight="bold" font-size="9pt" space-before="{$espacio-texto-celda-top}">
				<fo:inline color="{$txt-azul-claro}">
					<xsl:value-of select="$texto"/>
				</fo:inline>
			</fo:block>
