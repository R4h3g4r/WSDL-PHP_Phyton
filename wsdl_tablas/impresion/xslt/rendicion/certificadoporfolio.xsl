...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
									...
									<fo:table-cell>
										<fo:external-graphic width="70px" height="42px">
											<xsl:attribute name="src">
												<xsl:value-of select="/cliente/rutaImagen"/>logo-bci-somos-diferentes.gif
											</xsl:attribute>
										</fo:external-graphic>
									</fo:table-cell>
									<xsl:choose>
	...
	<xsl:choose>
		<xsl:when test="count(cliente/datos/detalle) > 0">
			...
			a usted que de acuerdo a las instrucciones impartidas por  la Empresa
				<xsl:value-of select="/cliente/razon"/>, Rut <xsl:value-of select="/cliente/rut"/>   y al amparo del convenio 
			de pago vigente con el Banco Crédito Inversiones por los Servicios de Pago de Remuneraciones, 
			se ha procedido a efectuar los siguientes pagos o abonos a las entidades, en las cuentas, en los montos 
			y en las fechas que a continuación se indica.</fo:block>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	...
	<xsl:choose>
		<xsl:when test="count(cliente/datos/detalle) > 0">
			...
			petición de <xsl:value-of select="/cliente/razon"/> y para los fines que estime 
			convenientes y sin ulterior responsabilidad para el Banco.</fo:block>
					...
					<fo:table table-layout="fixed">
						...
						<fo:table-body>
							<fo:table-row>
								...
								<fo:table-cell border="0.4pt solid #000000" background-color="{$bgcolor}">
									<fo:block font-size="8pt" text-align="center" font-weight="bold">Folio</fo:block>
								</fo:table-cell>
								<fo:table-cell border="0.4pt solid #000000" background-color="{$bgcolor}">
									<fo:block font-size="8pt" text-align="center" font-weight="bold">Monto total</fo:block>
								</fo:table-cell>
								<fo:table-cell border="0.4pt solid #000000" background-color="{$bgcolor}">
									<fo:block font-size="8pt" text-align="center" font-weight="bold">Monto Aceptado</fo:block>
								</fo:table-cell>
								<fo:table-cell border="0.4pt solid #000000" background-color="{$bgcolor}">
	...
	<xsl:otherwise>
		<fo:block padding-top="2cm" text-align="center">2.</fo:block>
	</xsl:otherwise>
</xsl:choose>
	</xsl:template>
