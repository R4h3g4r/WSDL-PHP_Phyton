...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="/">
			...
			<xsl:template name="textoInicio">
				<xsl:choose>
					<xsl:when test="count(cliente/datos/detalle) > 0">
						...
						<fo:block text-align="justify" padding-before="0.5cm">Por medio de la presente me permito informar a usted 
						que de acuerdo a las instrucciones impartidas por la Empresa <xsl:value-of select="/cliente/razon"/>, 
						Rut <xsl:value-of select="/cliente/rut"/>, y al amparo del convenio de pago vigente con el Banco Crédito Inversiones por los Servicios 
						de Pagos Electrónicos, se ha procedido a efectuar los siguientes pagos o abonos a las entidades, en las cuentas, 
						en los montos y en las fechas que a continuación se indica.</fo:block>
					</xsl:when>
			   </xsl:choose>
			</xsl:template>
		...
		<xsl:choose>
			<xsl:when test="count(cliente/datos/detalle) > 0">
				<fo:block text-align="justify" padding-before="0.5cm">Se extiende el presente certificado a expresa petición 
				de <xsl:value-of select="/cliente/razon"/> y para los fines que estime convenientes y sin ulterior 
				responsabilidad para el Banco.</fo:block>
					...
					<fo:table table-layout="fixed">
						...
						<fo:table-body>
							<fo:table-row>
								...
								<fo:table-cell border="0.4pt solid #000000" background-color="{$bgcolor}">
									<fo:block font-size="8pt" text-align="center" font-weight="bold">Cuenta Corriente</fo:block>
								</fo:table-cell>
								<fo:table-cell border="0.4pt solid #000000" background-color="{$bgcolor}">
									<fo:block font-size="8pt" text-align="center" font-weight="bold">Banco</fo:block>
								</fo:table-cell>
								<fo:table-cell border="0.4pt solid #000000" background-color="{$bgcolor}">
							...
							<xsl:for-each select="cliente/datos/detalle">
								<fo:table-row height="4mm">
									...
									<fo:table-cell border="0.4pt solid #000000">
										<fo:block font-size="8pt" text-align="center">
											<xsl:value-of select="/cliente/rutBeneficiario"/>
										</fo:block>
									</fo:table-cell>
									<fo:table-cell border="0.4pt solid #000000">
										<fo:block font-size="8pt" text-align="center">
