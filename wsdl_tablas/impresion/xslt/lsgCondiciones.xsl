	...
	<xsl:template name="contenido">
					...
					<fo:table-row>
						<fo:table-cell>
							...
							<fo:block font-size="15pt" font-weight="bold" text-align="left" color="{$txt-azul-claro}">
								Condiciones Línea de Sobregiro
							</fo:block>
							<!-- titulo -->
						</fo:table-cell>
					</fo:table-row>
		...
		<!-- tabla 2 Parte Inferior -->
			...
			<xsl:call-template name="subTitulo">
				<xsl:with-param name="titulo" select="'Características Linea de Sobregiro'"/>
			</xsl:call-template>
			<!-- Separador -->
			<fo:block space-after="2mm">
				<fo:leader color="{$lineas}" leader-pattern="rule" leader-length="18cm"/>
				...
				<fo:table-column column-width="18.0cm" />
						...
						<fo:table-row>
							...
							<fo:block>
								...
								&#0160;Renovación con Aumento de Cupo Automático 	
								</fo:inline>	
				...
				<fo:table-body>
					...
					<fo:table-row>
						<fo:table-cell border="0.5pt solid {$lineas}">
							<xsl:call-template name="itemTitulo">
								<xsl:with-param name="titulo" select="'Tipo Garantía:'"/>
							</xsl:call-template>
						</fo:table-cell>
						<fo:table-cell border="0.5pt solid {$lineas}">
							<fo:block font-weight="bold" color="{$txt-azul-oscuro}" padding-before="1mm" padding-after="1mm" text-align="left">
					...
					<fo:table-row>
						<fo:table-cell background-color="{$bgcolor}" border="0.5pt solid {$lineas}">
							<xsl:call-template name="itemTitulo">
								<xsl:with-param name="titulo" select="'Valor Comisión:'"/>
							</xsl:call-template>
						</fo:table-cell>
						<fo:table-cell background-color="{$bgcolor}" border="0.5pt solid {$lineas}">
							<fo:block font-weight="bold" color="{$txt-azul-oscuro}" padding-before="1mm" padding-after="1mm" text-align="left">
