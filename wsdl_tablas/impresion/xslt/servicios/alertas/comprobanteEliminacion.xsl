...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	<xsl:variable name="sello">
		<xsl:value-of select="comprobanteEliminacion/rutaImagen"/>sello_bci.gif
	</xsl:variable>
	<xsl:variable name="logo">
		<xsl:value-of select="comprobanteEliminacion/rutaImagen"/>logo_banco.gif
	</xsl:variable>
	<xsl:template match="/">
	...
	<fo:block text-aling="left" padding-top="-95pt" margin-left="500pt" space-after="15mm">
		Rut: <xsl:value-of select="comprobanteEliminacion/rut"/>
	</fo:block>	
	<fo:block text-aling="left" margin-left="" font-weight="bold" background-color="#A4A4A4" space-after="5mm" border="1pt solid" color="#FFFFFF">
		 &#0160;&#0160;Comprobante de Eliminaci&#243;n de Alertas
	</fo:block>
	<!-- Subtitulo del PDF -->	
	...
	<fo:table-column column-width="20cm" background-image="url({$sello})" background-repeat="no-repeat" padding-top="-100pt" padding-left="-20pt"/>
...
<!-- Columna que contiene el sello BCI -->
	...
	<fo:table-body border="1pt solid" width="21.5" padding-top="1pt">
		<fo:table-row>
			<fo:table-cell>
				<fo:block text-aling="left" margin-left="5pt" font-weight="lighter"  space-after="5mm" padding-top="5pt">
					La eliminaci&#243;n de sus alertas se ha realizado exitosamente. A continuaci&#243;n presentamos el detalle:
				</fo:block>
			</fo:table-cell>
		</fo:table-row>
		<fo:table-row>
		...
		<fo:table-row >
			<fo:table-cell>
				<fo:block text-aling="left" margin-left="15pt" font-weight="lighter">
					<xsl:value-of select="comprobanteEliminacion/producto"/>
				</fo:block>
			</fo:table-cell>
			<fo:table-cell>
				<fo:block  font-weight="lighter" margin-left="-350pt" > 
					<xsl:value-of select="comprobanteEliminacion/cuentaAlerta"/>
				</fo:block>
			</fo:table-cell>
			<fo:table-cell>
				<fo:block space-after="10mm" margin-left="-170pt"  font-weight="lighter">
					<!--<xsl:value-of select="pdfEliminacion/cuentaCargo"/>-->
				</fo:block>
			</fo:table-cell>
		</fo:table-row>	
		<fo:table-row>
		...
		<fo:table-row>
			<fo:table-cell>
				<fo:block font-weight="lighter" space-after="20mm" margin-left="250pt">
					Alertas eliminadas correctamente.
				</fo:block>
			</fo:table-cell>	
		</fo:table-row>	
			<xsl:for-each select="comprobanteEliminacion/eliminar">
				<fo:table-row>
					<fo:table-cell>
						<fo:block text-aling="left" padding-top="-35pt" margin-left="250pt" space-after="15mm">
							<xsl:value-of select="glosas"/>&#0160;
							<xsl:value-of select="mtoAdicional"/>
						</fo:block>
					</fo:table-cell>
				</fo:table-row>
			</xsl:for-each>	
				<xsl:if test="comprobanteEliminacion/eliminarError !=''">
					<fo:table-row>
			<fo:table-cell>				
				<fo:block text-aling="left"   font-weight="lighter" margin-left="250pt" space-after="15mm" >
					Alertas no eliminadas
				</fo:block>
			</fo:table-cell>	
		</fo:table-row>	
		
			<xsl:for-each select="comprobanteEliminacion/eliminarError">
				<fo:table-row>
					<fo:table-cell>
						<fo:block text-aling="left" padding-top="-35pt" margin-left="250pt" space-after="15mm">
							<xsl:value-of select="glosasError"/>&#0160;
		...
		<fo:table-row>
			<fo:table-cell>
				<fo:block text-align="left" margin-left="55pt" font-weight="lighter">
					<xsl:value-of select="comprobanteEliminacion/nombreCompleto"/>
				</fo:block>	
			</fo:table-cell>
			<fo:table-cell>
				<fo:block text-align="left" space-after="10mm"  margin-left="-185pt" font-weight="lighter">
					<xsl:value-of select="comprobanteEliminacion/telefono"/>
				</fo:block>	
			</fo:table-cell>	
		</fo:table-row>	
	</fo:table-body>
