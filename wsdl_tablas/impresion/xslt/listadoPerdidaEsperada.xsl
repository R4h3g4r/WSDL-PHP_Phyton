		...
		<fo:block>
			<fo:table table-layout="fixed">
				...
				<fo:table-body>
					<fo:table-row>
						<fo:table-cell>
							<fo:block font-size="15pt" font-weight="bold" text-align="left" color="{$txt-azul-claro}">Pérdida Esperada (PE)</fo:block>
						</fo:table-cell>
					</fo:table-row>
				</fo:table-body>
			</fo:table>
...
<fo:table-column column-width="10"/>
                    ...
                    <xsl:if test="count(Info/RiesgoCliente/RiesgoCliente)>0">
                        ...
                        <xsl:for-each select="Info/RiesgoCliente/RiesgoCliente">
                            <!--<xsl:if test="position()=1">-->
                                <fo:table-row>
                                    <xsl:call-template name="celda-centro">
						...
						<xsl:for-each select="Info/PerdidasEsperadas/PerdidaEsperada">
							<fo:table-row>
								...
								<xsl:call-template name="celda">
									<xsl:with-param name="texto" select="IndOrigen"/>
								</xsl:call-template>
								<xsl:call-template name="celda">
									<xsl:with-param name="texto" select="ValorPerdidaEsperadaTotal"/>
								</xsl:call-template>
								<xsl:call-template name="celda">
									<xsl:with-param name="texto" select="PorcePerdidaEsperadaTotal"/>
								</xsl:call-template>
								<xsl:call-template name="celda">
									<xsl:with-param name="texto" select="GloClasificacionSuper"/>
								</xsl:call-template>
							</fo:table-row>
						</xsl:for-each>
					</xsl:if>
    ...
    <xsl:template name="celda-centro">
        <xsl:param name="texto"/>
        <fo:table-cell border-top-style="solid" border-top-width="thin" border-top-color="{$borde-tabla}" border-bottom-style="solid" border-bottom-width="thin" border-bottom-color="{$borde-tabla}" border-left-style="solid" border-left-width="thin"
                       border-left-color="{$borde-tabla}" border-right-style="solid" border-right-width="thin" border-right-color="{$borde-tabla}">
