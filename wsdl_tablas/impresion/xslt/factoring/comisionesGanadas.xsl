...
<xsl:stylesheet version="1.0"
									...
									<fo:table-cell>
										<fo:external-graphic>
											<xsl:attribute name="src"><xsl:value-of
													select="comisiones-ganadas/cabecera/ruta-imagen" />02-logo-bci.gif
											</xsl:attribute>
										</fo:external-graphic>
									</fo:table-cell>
									<fo:table-cell
										...
										<fo:block font-size="9pt"
											...
											color="{$txt}">
											<xsl:value-of
												select="comisiones-ganadas/cabecera/fecha-impresion" />
										</fo:block>
									</fo:table-cell>
								</fo:table-row>
							</fo:table-body>
	...
	<xsl:template name="contenido">
					...
					<fo:table-row>
						<fo:table-cell>
							...
							<fo:block font-size="10pt"
								...
								<xsl:value-of
									select="comisiones-ganadas/cabecera/ejecutivo" />
							</fo:block>
							<!-- nombre cliente -->
						</fo:table-cell>
						<fo:table-cell>
							...
							<fo:block font-size="10pt"
								...
								<fo:inline color="{$txt-azul-claro}">
									<xsl:value-of
										select="comisiones-ganadas/cabecera/plataforma" />
								</fo:inline>
							</fo:block>
							<!-- rut cliente -->
						</fo:table-cell>
		...
		<xsl:call-template name="subTitulo">
			<xsl:with-param name="titulo" select="comisiones-ganadas/cabecera/grupo"/>
		</xsl:call-template>
		<!-- Separador -->
		<fo:block space-after="2mm">
			...
			<fo:table table-layout="fixed">
				...
				<fo:table-body>
					<fo:table-row>
						...
						<xsl:for-each select="comisiones-ganadas/periodos-comisiones/periodo">
							<fo:table-cell background-color="{$bgcolor}"
								border="0.5pt solid {$lineas}">
								<fo:block font-weight="normal"
									color="{$txt}" padding-before="1mm" padding-after="1mm"
					...
					<xsl:for-each select="comisiones-ganadas/ejecutivos/ejecutivo">
						<fo:table-row>
							<fo:table-cell
								border="0.5pt solid {$lineas}">
								<fo:block font-weight="normal"
							...
							<xsl:for-each select="periodos-comisiones/periodo">
								<fo:table-cell
									border="0.5pt solid {$lineas}">
									<fo:block font-weight="normal"
										color="{$txt}" padding-before="1mm" padding-after="1mm"
								...
								<fo:table-cell
									...
									<fo:block font-weight="normal"
										color="{$txt}" padding-before="1mm" padding-after="1mm"
										text-align="left">
										&#0160;<xsl:value-of select="monto-comision"/>
									</fo:block>
								</fo:table-cell>
							</xsl:for-each>
						</fo:table-row>
					...
					<fo:table-row>
						<fo:table-cell
							...
							<fo:block font-weight="bold"
								...
								text-align="left">
								&#0160;
								<xsl:value-of select="comisiones-ganadas/totales/banca" />
							</fo:block>
						</fo:table-cell>
						<xsl:for-each select="comisiones-ganadas/totales/totales-mensuales/total">
							<fo:table-cell
								border="0.5pt solid {$lineas}">
								<fo:block font-weight="bold"
									color="{$txt}" padding-before="1mm" padding-after="1mm"
						...
						<fo:table-cell padding-before="1mm">
							<fo:external-graphic height="1.2mm" width="1.2mm">
								<xsl:attribute name="src">
									<xsl:value-of select="comisiones-ganadas/cabecera/ruta-imagen"/>01-bullet2-rj.gif
								</xsl:attribute>
							</fo:external-graphic>
						</fo:table-cell>
						<fo:table-cell>
