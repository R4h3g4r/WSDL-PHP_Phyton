...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
						...
						<fo:table table-layout="fixed">
							...
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell>
										<fo:external-graphic>
											<xsl:attribute name="src"><xsl:value-of select="AnticipadoClienteEmpresa/cabecera/ruta-imagen"/>02-logo-bci.gif
											</xsl:attribute>
										</fo:external-graphic>
									</fo:table-cell>
									<fo:table-cell padding-before="1cm">
										<fo:block font-size="9pt" font-weight="bold" space-after="1mm" text-align="right" color="{$txt}">
											<xsl:value-of select="AnticipadoClienteEmpresa/cabecera/fecha-impresion"/>
										</fo:block>
									</fo:table-cell>
								</fo:table-row>
							</fo:table-body>
	...
	<xsl:template name="contenido">
					...
					<fo:table-row>
						<fo:table-cell>
							...
							<fo:block font-size="10pt" font-weight="bold" space-after="1mm" text-align="left" color="{$txt-azul-claro}">
								<xsl:value-of select="AnticipadoClienteEmpresa/cabecera/nombre"/>
							</fo:block>
							<!-- nombre Ejecutivo -->
						</fo:table-cell>
						<fo:table-cell>
							...
							<fo:block font-size="10pt" font-weight="bold" space-after="1mm" text-align="right" color="{$txt}">
								...
								<fo:inline color="{$txt-azul-claro}">
									<xsl:value-of select="AnticipadoClienteEmpresa/cabecera/Plataforma"/>
								</fo:inline>
							</fo:block>
							<!-- Plataforma -->
						</fo:table-cell>
		...
		<fo:block font-size="10pt" font-weight="bold" text-align="left" color="{$txt-azul-claro}">
			Resumen Total Banca Empresa
		</fo:block>
		<xsl:call-template name="tablasInferiores"/>
	</xsl:template>

<xsl:template name="tablasInferiores">
<xsl:for-each select="AnticipadoClienteEmpresa/tablaTotales/total">
<fo:block space-before="4mm">
	<fo:table table-layout="fixed">
		...
		<fo:table-body>
			...
			<xsl:for-each select="campo">
				<fo:table-cell border="0.5pt solid {$lineas}">
					<fo:block font-size="8pt" font-weight="normal" padding-before="1mm" padding-after="1mm" text-align="center" color="{$txt}">
...
<xsl:for-each select="AnticipadoClienteEmpresa/tablas/tabla">
<fo:block space-before="4mm">
	<fo:table table-layout="fixed">
		...
		<fo:table-body>
			...
			<fo:table-row>
				<fo:table-cell border="0.5pt solid {$lineas}">
					<fo:block font-size="8pt" font-weight="normal" padding-before="1mm" padding-after="1mm" text-align="left" color="{$txt}">
						&#0160;<fo:inline text-decoration="underline"><xsl:value-of select="nombre"/></fo:inline>
					</fo:block>
				</fo:table-cell>
